        ...
        <div className="flex-1 w-full relative">
          <ResponsiveContainer width="100%" height="100%">
            <BarChart data={monthlyChartData} margin={{ top: 10, right: 10, left: -20, bottom: 0 }}>
              ...
              <XAxis dataKey="month" axisLine={false} tickLine={false} tick={{ fill: '#c7c5d0', fontSize: 12 }} dy={10} />
      ...
      <div className="lg:col-span-1 glass-panel rounded-xl p-6 glass-panel-hover flex flex-col h-[400px]">
        ...
        <div className="flex-1 flex flex-col items-center justify-center relative">
          <ResponsiveContainer width="100%" height="100%">
            <PieChart>
              <Pie
                data={categoryData.length > 0 ? categoryData : [{ name: 'None', amount: 1, color: 'rgba(255,255,255,0.1)' }]}
                cx="50%"
                cy="50%"
                innerRadius={60}
                outerRadius={80}
              ...
              >
                {categoryData.length > 0
                  ? categoryData.map((entry, index) => (
                      <Cell key={`cell-${index}`} fill={entry.color || '#918f9a'} />
                    ))
