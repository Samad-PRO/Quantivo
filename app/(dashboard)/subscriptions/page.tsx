    ...
    <div style={{ fontFamily: 'Inter, sans-serif' }}>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '24px' }}>
        <div>
          ...
          <p style={{ fontSize: '14px', color: '#918f9a', margin: '4px 0 0' }}>Never get surprised by a renewal.</p>
        </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '16px', marginBottom: '24px' }}>
        {[
          ...
          <div key={label as string} style={{ background: 'rgba(18,33,49,0.6)', border: '1px solid rgba(255,255,255,0.08)', borderRadius: '16px', padding: '20px' }}>
            <p style={{ fontSize: '12px', color: '#918f9a', textTransform: 'uppercase', letterSpacing: '0.05em', marginBottom: '8px' }}>{label}</p>
      ...
      <div style={{ display: 'flex', gap: '8px', marginBottom: '20px' }}>
        {(['monthly', 'yearly'] as const).map(b => (
          <button key={b} onClick={() => setBilling(b)}
            style={{ padding: '6px 16px', borderRadius: '999px', border: '1px solid', borderColor: billing === b ? '#c0c1ff' : 'rgba(255,255,255,0.1)', background: billing === b ? 'rgba(192,193,255,0.1)' : 'transparent', color: billing === b ? '#c0c1ff' : '#918f9a', cursor: 'pointer', fontSize: '13px', fontWeight: 500, textTransform: 'capitalize' }}>
            {b}
          </button>
        ))}
      </div>
        ...
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.7)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 100, padding: '16px' }}>
          <div style={{ background: 'var(--bg-surface)', border: '1px solid rgba(255,255,255,0.1)', borderRadius: '16px', padding: '32px', width: '100%', maxWidth: '440px' }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '14px' }}>
              ...
              <div><label style={{ fontSize: '12px', color: '#918f9a', display: 'block', marginBottom: '6px' }}>Amount (USD)</label><input style={inp} type="number" value={form.amount} onChange={e => setForm(f => ({ ...f, amount: e.target.value }))} placeholder="15.99" /></div>
              <div>
                <label style={{ fontSize: '12px', color: '#918f9a', display: 'block', marginBottom: '6px' }}>Billing Cycle</label>
              ...
              <div>
                <label style={{ fontSize: '12px', color: '#918f9a', display: 'block', marginBottom: '6px' }}>Category</label>
      ...
      {loading ? (
        ...
        <div style={{ textAlign: 'center', padding: '80px 24px', background: 'rgba(18,33,49,0.4)', borderRadius: '16px', border: '1px solid rgba(255,255,255,0.06)' }}>
          ...
          <p style={{ color: '#918f9a', fontSize: '14px', marginBottom: '24px' }}>Add your recurring services to track spending</p>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(260px, 1fr))', gap: '16px' }}>
          {subs.map(sub => {
            ...
            return (
              <div key={sub.id} style={{ background: 'rgba(18,33,49,0.6)', border: '1px solid rgba(255,255,255,0.08)', borderRadius: '16px', padding: '20px', transition: 'all 0.2s' }}>
                ...
                <div style={{ display: 'flex', alignItems: 'flex-end', justifyContent: 'space-between' }}>
                  <div>
                    ...
                    <p style={{ fontSize: '11px', color: '#918f9a', margin: '2px 0 0' }}>/{billing === 'yearly' ? 'year' : 'month'}</p>
                  </div>
                  {daysUntil !== null && (
                    <div style={{ background: daysUntil <= 7 ? 'rgba(255,180,171,0.15)' : 'rgba(192,193,255,0.08)', borderRadius: '8px', padding: '4px 10px' }}>
                      <p style={{ fontSize: '11px', color: daysUntil <= 7 ? '#ffb4ab' : '#918f9a', margin: 0, fontWeight: 600 }}>
